...
switch ($_GET["op"]){
    case 'guardaryeditar':
        ...
        if (empty($art_codigo)){
            ...
            $rspta=$articulov->editar($art_codigo,$art_modelo,$art_color,$art_talla,$art_marca,
                $art_nombre,$art_unidad, $art_caracteristica, $art_stock_min, $art_stock_max);
            echo $rspta ? "Artículo actualizado" : "Artículo no se pudo actualizar";
        }
    break;

    case 'desactivar':
        $rspta=$articulov->desactivar($art_codigo);
        echo $rspta ? "Artículo Desactivado" : "Artículo no se puede desactivar";
    break;

    case 'activar':
        $rspta=$articulov->activar($art_codigo);
        echo $rspta ? "Artículo Activado" : "Artículo no se puede activar";
    break;

    case 'mostrar':
        $rspta=$articulov->mostrar($art_codigo);
    ...
    case 'listar':
        ...
        while ($reg=$rspta->fetch_object()){
            $data[]=array(
                "0"=>($reg->art_estado=='ACTIVO')?'<button class="btn btn-warning" type="text" onclick="mostrar('.$reg->art_codigo.')"><i class="fa fa-pencil"></i></button>'.
